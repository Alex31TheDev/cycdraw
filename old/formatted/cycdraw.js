...
class Point {
    ...
    *[Symbol.iterator]() {
    ...
    complexDiv(p) {
        ...
        return new Point((this.x * p.x - this.y * p.y) / sum, (this.x * p.y + this.y * p.x) / sum);
    }
}
...
function writeImg() {
    ...
    img.fillBarycentric = function (x1, y1, x2, y2, x3, y3, color) {
        if (
            (x1 < 0 && x2 < 0 && x3 < 0) ||
            (x1 > this.w && x2 > this.w && x3 > this.w) ||
            (y1 < 0 && y2 < 0 && y3 < 0) ||
        ...
        let fx1 = Math.max(Math.min(x1, Math.min(x2, x3)), 0);
        let fy1 = Math.max(Math.min(y1, Math.min(y2, y3)), 0);
        let fx2 = Math.min(Math.max(x1, Math.max(x2, x3)), this.w - 1);
        let fy2 = Math.min(Math.max(y1, Math.max(y2, y3)), this.h - 1);
        ...
        let c1 = -(a1 * (x1 + x2) + b1 * (y1 + y2)) / 2,
            c2 = -(a2 * (x2 + x3) + b2 * (y2 + y3)) / 2,
            c3 = -(a3 * (x1 + x3) + b3 * (y1 + y3)) / 2;

        let t1 = a1 != 0 ? a1 > 0 : b1 > 0,
            t2 = a2 != 0 ? a2 > 0 : b2 > 0,
            t3 = a3 != 0 ? a3 > 0 : b3 > 0;
        ...
        for (let x = fx1; x <= fx2; x++) {
            for (let y = fy1; y <= fy2; y++) {
                ...
                if ((v1 > 0 || v1 == 0) && (v2 > 0 || v2 == 0) && (v3 > 0 || v3 == 0)) {
    ...
    img.fillBarycentric(img.w / 2, 20, img.w / 2 + 100, img.h / 2, img.w / 2 + 200, img.h / 2 - 100, Colors.red);
...
let benchmark = {};
let amogstr = "";
//let tag = {args: "-mangus us -bamogus 69 -susus -pamus 69 alungus -gus333 \"69\" \`\`\`us\`\`\`"}
//let msg = {attachments: ""}
